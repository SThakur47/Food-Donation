...
public class ProjectDaoImpl implements ProjectDao { 
	...
	public boolean verifyReceiverOTPAndDeleteRecord(String email, int otp) {
		...
		Query query = manager.createQuery("from FoodUnit where contactNumber=:phone");
		query.setParameter("phone", orders.getDonorPhone());
		FoodUnit unit = (FoodUnit) query.getResultList().get(0);
		int unitId = unit.getUnitId();
		FoodUnit foodUnit  = manager.find(FoodUnit.class, unitId);
		try {
			if(orders.getReceiverOtp() == otp) {
				transaction.begin();
				manager.remove(orders);
				manager.remove(foodUnit);
				transaction.commit();
				return true;
			}else {
				return false;
